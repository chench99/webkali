  ...
  <div class="wifi-container p-6 bg-gray-900 text-white min-h-screen font-mono">
    ...
    <div class="panel-header flex justify-between items-end mb-6 border-b border-gray-700 pb-4">
      <div>
        ...
        <div class="text-gray-500 text-xs mt-1">Kali Linux C2 Platform v2.0 | 深度扫描模块</div>
      </div>
    ...
    <div class="toolbar flex justify-between items-center mb-4">
      <div class="search-box relative">
        ...
        <input 
          v-model="searchQuery"
          type="text" 
          placeholder="过滤 SSID / BSSID / 厂商..." 
          class="bg-gray-800 border border-gray-600 text-white pl-10 pr-4 py-2 rounded focus:outline-none focus:border-blue-500 w-64 text-sm"
        >
      </div>

      <div class="controls flex gap-3">
        <div class="text-xs text-gray-500 self-center mr-2">
          发现: {{ filteredNetworks.length }} / {{ networks.length }}
        </div>
        <button 
      ...
      <table class="w-full text-left text-sm">
        <thead class="bg-gray-900 text-gray-400 uppercase tracking-wider">
          <tr>
            ...
            <th class="p-4 border-b border-gray-700">厂商 (Vendor)</th> <th class="p-4 border-b border-gray-700 cursor-pointer hover:text-white" @click="sortBy('channel')">信道/频段 ⇅</th> <th class="p-4 border-b border-gray-700 cursor-pointer hover:text-white" @click="sortBy('encryption')">加密 ⇅</th>
        ...
        <tbody class="divide-y divide-gray-700">
          <tr v-if="filteredNetworks.length === 0 && !isScanning">
            <td colspan="8" class="p-12 text-center text-gray-500">
              ...
              <div>{{ networks.length > 0 ? '未找到匹配结果' : '暂无目标，请初始化扫描' }}</div>
            </td>
          </tr>
  ...
  methods: {
    async fetchSystemStatus() {
      try {
        ...
        if (res.data) this.systemStatus = res.data;
    ...
    async startScan() {
      ...
      try {
        ...
        if (this.networks.length > 0) {
          this.$message?.success(`扫描完成，更新 ${this.networks.length} 个目标`);
        } else {
          this.$message?.warning("未发现目标，请检查天线连接");
        }
      } catch (error) {
    ...
    async attackTarget(net) {
      if(!confirm(`[WARNING]\n确认对 ${net.ssid} (${net.bssid}) 执行 Deauth 攻击？\n此操作将断开目标网络连接。`)) return;
      try {
        await axios.post('/api/v1/wifi/capture/start', {
          bssid: net.bssid, channel: net.channel, attack_type: 'deauth'
        });
        alert("🚀 攻击指令已发送至后台！");
      } catch(e) {
        alert("❌ 攻击请求失败: " + e.message);
      }
    },
    ...
    getSignalPercent(signal) {
      let dbm = parseInt(signal) || -100;
      if (dbm > -50) return 100;
      if (dbm < -100) return 0;
      return Math.min(100, Math.max(0, (dbm + 100) * 2));
    },
    getSignalColor(signal) {
      let p = this.getSignalPercent(signal);
    ...
    getIntelligence(net) {
      let tags = [];
      const enc = (net.encryption || "").toUpperCase();
      ...
      if (vendor.includes("tp-link")) tags.push(`<span class="text-blue-300 text-xs">🌐 Router</span>`);

      // 信号评估
      if (parseInt(net.signal) > -50) tags.push(`<span class="text-green-400 text-xs">📍 Near</span>`);
      if (parseInt(net.signal) < -85) tags.push(`<span class="text-gray-600 text-xs">Edge</span>`);
